...
include("../db_kassa.php");
    ...
    try {
        ...
        foreach ($cl as $value) {
            ...
            if ($farq > 0){
                $res = $con->query("select client_id, DATE(sana) as sana from muddati_o_tani WHERE client_id='{$value["id"]}' and status=0 ORDER BY id ASC LIMIT 1");
                ...
                if (!$res1) throw new Exception($con->error, 1);
            }
        }
        $con->commit();
        ...
        fwrite($fp2, $e.'=> '.date("l jS \of F Y h:i:s A")."\n");
        fclose($fp2);
    }
